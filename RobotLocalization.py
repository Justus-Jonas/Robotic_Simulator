...
FeatureTuple = namedtuple("Feature", ["r", "phi", "s","px","py"])

EllipseTuple = namedtuple("Ellipse",["cx","cy","w","h","a"])
...
class LRobot:
    ...
    def UpdateSensor(self,beacon):
        ...
        if isDetected:
            ...
            temp = temp + sigmas
            f = FeatureTuple(temp[0],temp[1], temp[2],beacon.pos[0],beacon.pos[1])
    ...
    def AddEstimatedPath(self, mean, covariance,allowAddEllipse):
        ...
        if self.prevEPos[0] != mean[0] or self.prevEPos[1] != mean[1]:
                ...
                if allowAddEllipse:
                    ...
                    pos = np.copy(mean)
                    e = EllipseTuple(pos[0],pos[1],width,height,theta)
